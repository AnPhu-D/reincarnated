...
Entity::Entity(std::string _name, int _totalHealth, int _defense, int _attack, std::vector<Skill *> skills, std::vector<std::vector<int>> _image)
{
    name = _name;
    totalHealth = _totalHealth;
    currentHealth = _totalHealth;
...
{
    std::cout << name << " " << currentHealth << "/" << totalHealth;
}

void Entity::printEntityDetail()
{
    std::cout << "Name: " << name << "  Lv." << level << " Defense: " << defense << " Attack: " << attack << " " << currentHealth << "/" << totalHealth;
}
